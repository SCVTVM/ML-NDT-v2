...
iscrack = layers.Dense(1, activation='relu', name='output')(cb)


model = Model(input_tensor, iscrack)
# opt = keras.optimizers.RMSprop(learning_rate=0.0001, clipnorm=1.0)
...
print(ys)
model.fit(data_generator(100), epochs=100, validation_data = (xs,  ys[:, 1]), steps_per_epoch=60, callbacks=callbacks)
